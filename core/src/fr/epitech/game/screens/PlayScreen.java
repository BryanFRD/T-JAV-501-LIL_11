...
public class PlayScreen implements Screen {
    ...
    public void update(float delta){
        float cameraX = Math.max(entityManager.getPlayer().getCoordinate().x, EpiGame.V_WIDTH / EpiGame.PPM / 2f);
        float cameraY = Math.max(Math.min(entityManager.getPlayer().getCoordinate().y, Chunk.SIZE_Y * Chunk.TILE_SIZE - EpiGame.V_HEIGHT / EpiGame.PPM), EpiGame.V_HEIGHT / EpiGame.PPM / 2f);

        camera.position.slerp(new Vector3(cameraX, cameraY, 0), 0.5f);
        camera.update();
